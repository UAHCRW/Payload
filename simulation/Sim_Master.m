...
for i=1:size(acc,1)-1
   ref_velocity(i+1,:)=ref_velocity(i,:)+acc(i,:)*(t(i+1)-t(i));
end

for i=1:size(acc,1)-1
   ref_position(i+1,:)=ref_position(i,:)+ref_velocity(i,:)*(t(i+1)-t(i));
end
...
acc_out=cell(numruns,1);%initialize storage array
gyro_out=cell(numruns,1);%initialize storage array

acc_data=cell(numruns,1);%initialize storage array
vel_data=cell(numruns,1);%
pos_data=cell(numruns,1);%
