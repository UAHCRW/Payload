...
pos_init=[0,0,0];
vel_init=[0,0,0];
C_init=[1,0,0; 0,1,0; 0,0,1];

%define acceleration functions
%NOTE: define constants like t-t+constant to get an array
x_acc_fun=@(t) t-t+1;
y_acc_fun=@(t) t-t+0;
z_acc_fun=@(t) t-t+0;
acc=[x_acc_fun(times),y_acc_fun(times),z_acc_fun(times)];


%define angular velocity based on rpm
rpm=60;%rotating once every second
rad_per_second=2*pi*rpm*runtime/60;%find total distance traversed

%define angular velocity functions
%NOTE: define constants like t-t+constant to get an array
x_angVel_fun=@(t) t-t+0;
y_angVel_fun=@(t) t-t+0;
z_angVel_fun=@(t) t-t+rad_per_second;
angVel=[x_angVel_fun(times),y_angVel_fun(times),z_angVel_fun(times)];
...
ref_velocity(1,:)=vel_init;
ref_position(1,:)=pos_init;
...
%last measured trajectory with the reference trajectory
% figure
% hold on
% plot3(position(:,1),position(:,2),position(:,3))
% plot3(ref_position(:,1),ref_position(:,2),ref_position(:,3))
% xlabel('x')
% ylabel('y')
% zlabel('z')
% legend('Measured Location','Actual Location','location','best')