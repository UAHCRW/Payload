...
rpm=2;%rotating once every second
...
Rat_Mats=cell(num_runs,1);%initialize storage array
...
for ii=1:num_runs
    ...
    orientation=zeros(size(gyro_output));
    for i=1:size(gyro_output,1)-1
        orientation(i+1,:)=orientation(i,:)+gyro_output(i,:).*(times(i+1)-times(i));
    end
    %orientation(:,2)=pi/3;
    %initialize
    acceleration=zeros(size(accelerometer_output));
    gamma=-1*orientation(:,1);
    beta=-1*orientation(:,2);
    alpha=-1*orientation(:,3);
    %rotate
    for i=1:size(accelerometer_output,1)%for each row in the output, rotate
        %create matrix
        Rat_Mat=[cos(alpha(i)).*cos(beta(i)), cos(alpha(i)).*sin(beta(i)).*sin(gamma(i))-sin(alpha(i)).*cos(gamma(i)), cos(alpha(i)).*sin(beta(i)).*cos(gamma(i))+sin(alpha(i)).*sin(gamma(i));...
            sin(alpha(i)).*cos(beta(i)), sin(alpha(i)).*sin(beta(i)).*sin(gamma(i))+cos(alpha(i)).*cos(gamma(i)), sin(alpha(i)).*sin(beta(i)).*cos(gamma(i))-cos(alpha(i)).*sin(gamma(i));...
            -1*sin(beta(i)), cos(beta(i)).*sin(gamma(i)),cos(beta(i)).*cos(gamma(i))];
        Rat_Mats{i}=Rat_Mat;
        %rotate
        dummy=Rat_Mat*accelerometer_output(i,:)';
        %dummy=Rat_Mat\accelerometer_output(i,:)';
