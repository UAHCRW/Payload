...
{
    short status;
    if (mpu_get_int_status(&status) == INV_SUCCESS) { return status; }
    return 0;
}
...
inv_error_t MPU9250_DMP::setGyroFSR(unsigned short fsr)
{
    ...
    if (err == INV_SUCCESS) { _gSense = getGyroSens(); }
    return err;
}
...
inv_error_t MPU9250_DMP::setAccelFSR(unsigned char fsr)
{
    ...
    if (err == INV_SUCCESS) { _aSense = getAccelSens(); }
    return err;
}
...
{
    unsigned short tmp;
    if (mpu_get_gyro_fsr(&tmp) == INV_SUCCESS) { return tmp; }
    return 0;
}
...
{
    unsigned char tmp;
    if (mpu_get_accel_fsr(&tmp) == INV_SUCCESS) { return tmp; }
    return 0;
}
...
{
    unsigned short tmp;
    if (mpu_get_compass_fsr(&tmp) == INV_SUCCESS) { return tmp; }
    return 0;
}
...
{
    unsigned short tmp;
    if (mpu_get_lpf(&tmp) == INV_SUCCESS) { return tmp; }
    return 0;
}
...
{
    unsigned short tmp;
    if (mpu_get_sample_rate(&tmp) == INV_SUCCESS) { return tmp; }
    return 0;
}
...
{
    unsigned short tmp;
    if (mpu_get_compass_sample_rate(&tmp) == INV_SUCCESS) { return tmp; }
...
{
    float sens;
    if (mpu_get_gyro_sens(&sens) == INV_SUCCESS) { return sens; }
    return 0;
}
...
{
    unsigned short sens;
    if (mpu_get_accel_sens(&sens) == INV_SUCCESS) { return sens; }
    return 0;
}
...
unsigned char MPU9250_DMP::getFifoConfig(void)
{
    ...
    if (mpu_get_fifo_config(&sensors) == INV_SUCCESS) { return sensors; }
    return 0;
}
...
unsigned short MPU9250_DMP::fifoAvailable(void)
{
    ...
    if (mpu_read_reg(MPU9250_FIFO_COUNTH, &fifoH) != INV_SUCCESS) return 0;
    if (mpu_read_reg(MPU9250_FIFO_COUNTL, &fifoL) != INV_SUCCESS) return 0;
...
inv_error_t MPU9250_DMP::updateFifo(void)
{
    ...
    if (mpu_read_fifo(gyro, accel, &timestamp, &sensors, &more) != INV_SUCCESS) return INV_ERROR;
    ...
    if (sensors & INV_X_GYRO) gx = gyro[X_AXIS];
    if (sensors & INV_Y_GYRO) gy = gyro[Y_AXIS];
    if (sensors & INV_Z_GYRO) gz = gyro[Z_AXIS];
...
inv_error_t MPU9250_DMP::update(unsigned char sensors)
{
    ...
    if (sensors & UPDATE_TEMP) tErr = updateTemperature();
...
int MPU9250_DMP::updateAccel(void)
{
    ...
    if (mpu_get_accel_reg(data, &time)) { return INV_ERROR; }
    ax = data[X_AXIS];
    ay = data[Y_AXIS];
    az = data[Z_AXIS];
    return INV_SUCCESS;
...
int MPU9250_DMP::updateGyro(void)
{
    ...
    if (mpu_get_gyro_reg(data, &time)) { return INV_ERROR; }
    gx = data[X_AXIS];
    gy = data[Y_AXIS];
    gz = data[Z_AXIS];
    return INV_SUCCESS;
...
int MPU9250_DMP::updateCompass(void)
{
    ...
    int err = mpu_get_compass_reg(data, &time);
...
inv_error_t MPU9250_DMP::dmpBegin(unsigned short features, unsigned short fifoRate)
{
    ...
    if (dmpLoad() != INV_SUCCESS) return INV_ERROR;
    ...
    if (feat & DMP_FEATURE_GYRO_CAL) dmp_enable_gyro_cal(1);

    if (dmpEnableFeatures(feat) != INV_SUCCESS) return INV_ERROR;

    rate = constrain(rate, 1, 200);
    if (dmpSetFifoRate(rate) != INV_SUCCESS) return INV_ERROR;
...
{
    unsigned short rate;
    if (dmp_get_fifo_rate(&rate) == INV_SUCCESS) return rate;
...
inv_error_t MPU9250_DMP::dmpUpdateFifo(void)
{
    ...
    if (dmp_read_fifo(gyro, accel, quat, &timestamp, &sensors, &more) != INV_SUCCESS) { return INV_ERROR; }
...
{
    unsigned short mask;
    if (dmp_get_enabled_features(&mask) == INV_SUCCESS) return mask;
    return 0;
}
                                   ...
                                   unsigned char taps, unsigned short tapTime, unsigned short tapMulti)
{
    ...
    if (xThresh > 0)
    {
        ...
        if (dmp_set_tap_thresh(1 << X_AXIS, xThresh) != INV_SUCCESS) return INV_ERROR;
    }
    if (yThresh > 0)
    {
        axes |= TAP_Y;
        yThresh = constrain(yThresh, 1, 1600);
        if (dmp_set_tap_thresh(1 << Y_AXIS, yThresh) != INV_SUCCESS) return INV_ERROR;
    }
    if (zThresh > 0)
    {
        axes |= TAP_Z;
        zThresh = constrain(zThresh, 1, 1600);
        if (dmp_set_tap_thresh(1 << Z_AXIS, zThresh) != INV_SUCCESS) return INV_ERROR;
    }
    if (dmp_set_tap_axes(axes) != INV_SUCCESS) return INV_ERROR;
    if (dmp_set_tap_count(taps) != INV_SUCCESS) return INV_ERROR;
    if (dmp_set_tap_time(tapTime) != INV_SUCCESS) return INV_ERROR;
    if (dmp_set_tap_time_multi(tapMulti) != INV_SUCCESS) return INV_ERROR;
...
inv_error_t MPU9250_DMP::dmpEnable3Quat(void)
{
    ...
    if (dmpEnableFeatures(dmpFeatures) != INV_SUCCESS) return INV_ERROR;
...
{
    unsigned long steps;
    if (dmp_get_pedometer_step_count(&steps) == INV_SUCCESS) { return steps; }
    return 0;
}
...
unsigned long MPU9250_DMP::dmpGetPedometerTime(void)
{
    ...
    if (dmp_get_pedometer_walk_time(&walkTime) == INV_SUCCESS) { return walkTime; }
    return 0;
}
