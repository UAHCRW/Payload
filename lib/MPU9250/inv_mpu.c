...
int mpu_set_gyro_bias_reg(long* gyro_bias)
{
    ...
    int i                 = 0;
    for (i = 0; i < 3; i++) { gyro_bias[i] = (-gyro_bias[i]); }
...
int mpu_get_accel_sens(unsigned short* sens)
{
    switch (st.chip_cfg.accel_fsr)
    {
        ...
        case INV_FSR_4G: sens[0] = 8192; break;
...
static int accel_6500_self_test(long* bias_regular, long* bias_st, int debug)
{
    ...
    if (result == 0)
    {
        ...
        for (i = 0; i < 3; i++)
        {
            ...
            {
                // if (debug) log_i("FAILED: Accel axis:%d = %ld > 500mg\n", i, bias_regular[i]);
                result |= 1 << i; // Error condition
            }
        }
    }
...
static int gyro_6500_self_test(long* bias_regular, long* bias_st, int debug)
{
    ...
    if (result == 0)
    {
        ...
        for (i = 0; i < 3; i++)
        {
            ...
            {
                // if (debug) log_i("FAILED: Gyro axis:%d = %ld > 20dps\n", i, bias_regular[i]);
                result |= 1 << i; // Error condition
            }
        }
    }
...
#    elif defined AK8963_SECONDARY
    ...
    if (!(tmp[0] & AKM_DATA_READY)) return -2;
    if (!(tmp[0] & AKM_DATA_OVERRUN)) return -7;
